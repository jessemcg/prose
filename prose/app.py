...
from .windows.editor_commands import EditorCommandsWindow
from .windows.settings import SettingsWindow


def _sync_uno_globals() -> None:
    global uno, PropertyValue, NoConnectException, XModel, ControlCharacter, XTextDocument

    uno = _runtime.uno
    PropertyValue = _runtime.PropertyValue
    NoConnectException = _runtime.NoConnectException
    XModel = _runtime.XModel
    ControlCharacter = _runtime.ControlCharacter
    XTextDocument = _runtime.XTextDocument
...
"""
    ...
    def _launch_writer_document(self, path: Path) -> None:
        ...
        if not desktop:
            if uno is None:
                message = "python-uno unavailable. Set the LibreOffice Python path in Settings."
            else:
                message = "LibreOffice listener unavailable; cannot open document."
            print(message, file=sys.stderr)
            self._show_toast(message)
            return
        try:
            ...
            print(f"Opened {path} in LibreOffice.", file=sys.stderr)
        except Exception as exc:  # noqa: BLE001
    ...
    def _on_view_model_response_clicked(self, _button: Gtk.Button | None) -> None:
        ...
        view = Adw.ToolbarView()
        header = Adw.HeaderBar()
        header.set_title_widget(Adw.WindowTitle(title="Model Response Audit"))
    ...
    def _on_view_editor_prompt_clicked(self, _button: Gtk.Button | None) -> None:
        ...
        view = Adw.ToolbarView()
        header = Adw.HeaderBar()
        header.set_title_widget(Adw.WindowTitle(title="Latest LLM Prompt"))
        header.set_show_end_title_buttons(True)
        view.add_top_bar(header)
        scroller = Gtk.ScrolledWindow()
        scroller.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
    ...
    def _start_listener(self) -> None:
        ...
        try:
            ...
            message = f"Unable to start LibreOffice: {exc}"
            print(message, file=sys.stderr)
            self._show_toast(message)
...
def main(argv: list[str] | None = None) -> int:
    app = ProseApp()
    run_argv = sys.argv if argv is None else [sys.argv[0], *argv]
    status = int(app.run(run_argv))
